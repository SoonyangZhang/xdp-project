#include <unistd.h>
#include "../../lib/log/my_log.h"
#include "ncx_slab.h"

DEFINE_LOG_CATEGORY("ncx_memory");
...
ncx_slab_free_locked(ncx_slab_pool_t *pool, void *p)
{
    ...
    log_debug("slab free: %p\n", p);

    if ((u_char *) p < pool->start || (u_char *) p > pool->end) {
        log_error("ncx_slab_free(): outside of pool\n");
        goto fail;
    }
    ...
    case NCX_SLAB_PAGE:
		...
		if (slab == NCX_SLAB_PAGE_FREE) {
			log_warn("ncx_slab_free(): page is already free\n");
			goto fail;
        }

		if (slab == NCX_SLAB_PAGE_BUSY) {
			log_warn("ncx_slab_free(): pointer to wrong page\n");
			goto fail;
        }
...
wrong_chunk:
	...
	log_error("ncx_slab_free(): pointer to wrong chunk\n");

    goto fail;

chunk_already_free:

	log_error("ncx_slab_free(): chunk is already free\n");
...
ncx_slab_alloc_pages(ncx_slab_pool_t *pool, ncx_uint_t pages)
{
    ...
    log_error("ncx_slab_alloc() failed: no memory\n");
...
ncx_slab_stat(ncx_slab_pool_t *pool, ncx_slab_stat_t *stat)
{
	...
	log_info("pool_size : %zu bytes\n",	stat->pool_size);
	log_info("used_size : %zu bytes\n",	stat->used_size);
	log_info("used_pct  : %zu%%\n",		stat->used_pct);

	log_info("total page count : %zu\n",	stat->pages);
	log_info("free page count  : %zu\n",	stat->free_page);
		
	log_info("small slab use page : %zu,\tbytes : %zu\n",	stat->p_small, stat->b_small);	
	log_info("exact slab use page : %zu,\tbytes : %zu\n",	stat->p_exact, stat->b_exact);
	log_info("big   slab use page : %zu,\tbytes : %zu\n",	stat->p_big,   stat->b_big);	
	log_info("page slab use page  : %zu,\tbytes : %zu\n",	stat->p_page,  stat->b_page);				

	log_info("max free pages : %zu\n",		stat->max_free_pages);
}
...
void ncx_slab_module_init(void){
    LOG_ENSURE_REGISTER();
}
