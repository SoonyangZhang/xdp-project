...
static void parse_command_line(int argc, char **argv)
{
	...
	if (!opt_ifindex) {
		log_error("ERROR: interface \"%s\" does not exist\n",
			opt_if);
		usage(basename(argv[0]));
	}
}
...
static void l2fwd(struct xdpsock *xsk)
{
    ...
    while(g_running){
        ...
        for (i = 0; i < rcvd; i++) {
            char *pkt = xq_get_data(xsk, descs[i].addr);
            packet_dump(pkt,descs[i].len);
            packet_icmp_echo(pkt,descs[i].len);
        }
        xsk->rx_npkts += rcvd;
...
//./xdpsock -l -i wlxe0e1a997ab49 -q 0 -n 5 -S
// sudo ip link set wlxe0e1a997ab49 xdpgeneric off
int main(int argc, char **argv)
{
    main_module_init();
    packet_module_init();
    log_category_enable("main",LOG_ALL);
    log_category_enable("packet",LOG_ALL);
    ...
    if (setrlimit(RLIMIT_MEMLOCK, &r)) {
        log_error("setrlimit(RLIMIT_MEMLOCK) \"%s\"\n",strerror(errno));
        exit(EXIT_FAILURE);
    }
	...
	if(opt_ifindex){
        ...
        if(map){
            ...
            log_info("finding %s failed\n",qidconf_map_name);
            bpf_object__close(obj);
            return 1;
        }
        qidconf_map = bpf_map__fd(map);
        if(qidconf_map < 0){
            log_error("no qidconf map found: %s\n",strerror(qidconf_map));
            bpf_object__close(obj);
            return 1;
        }
        ...
        if (xsks_map < 0) {
            log_error("no xsks map found: %s\n",strerror(xsks_map));
            bpf_object__close(obj);
            return 1;
        }
    }else{
        log_error("nic does not exist\n");
        bpf_object__close(obj);
        return 1;
    }
    int prog_id=-1;
	...
	if (ret) {
		log_error("bpf_map_update_elem qidconf\n");
		bpf_object__close(obj);
		exit(EXIT_FAILURE);
	}
	...
	for (i = 0; i < num_socks; i++) {
		...
		if (ret) {
			log_error("bpf_map_update_elem %d\n", i);
			bpf_object__close(obj);
			exit(EXIT_FAILURE);
		}
	}
